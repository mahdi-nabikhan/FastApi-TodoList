...
def create_task(request:TaskCreateSchemas,db:Session=Depends(get_db),user:UserModel=Depends(get_authenticated_user)):
...
def update_task(request:TaskCreateSchemas,id:int,db:Session=Depends(get_db),user:UserModel=Depends(get_authenticated_user)):
...
def detail_task(id:int,db:Session=Depends(get_db),user:UserModel=Depends(get_authenticated_user)):
...
def delete_task(id:int,db:Session=Depends(get_db),user:UserModel=Depends(get_authenticated_user)):
    tasks = db.query(TaskModel).filter_by(user_id=user.id ,id=id).first()
    if not tasks:
        raise HTTPException(status_code=404,detail='Task not found')
    db.delete(tasks)
